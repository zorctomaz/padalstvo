 ...
 * 24h) - izloči samo znane pokvarjene privzete koordinate (altitude 0,
 * glej SKYTECH_API_ISSUES.md), ker bi te sicer prikazale postajo na
 * povsem napačni lokaciji. Postaje brez sveže meritve so vizualno
 * ločene (sivi, bolj prosojen pin - .map-pin-station-stale), da je jasno,
 * da trenutno morda ne poročajo; klik nanje še vedno odpre podrobnosti
 * (zadnja znana meritev, če obstaja).
 */
...
/**
 * Veter (in temperatura) po višini (tlačni nivoji/hPa) - ARSO tega ne
 * objavlja strojno berljivo (preverjeno prek GitHub Actions: napovedni
 * API vrne le prizemne vrednosti, letalska stran SIGWX/GAFOR ponuja le
 * grafične/besedilne produkte). Open-Meteo (api.open-meteo.com) je
 * brezplačen, brez API ključa, z odprtim CORS-om (preverjeno prek GitHub
 * Actions - Access-Control-Allow-Origin: *), zato ga pokličemo neposredno
 * iz brskalnika, brez strežniške predpriprave (poljubna GPS točka pri
 * "Moja lokacija" je ni mogoče vnaprej zgraditi za vsako možnost).
 *
 * Prikaz posnema tabelo iz ARSO-jeve gorske vremenske aplikacije
 * ("Vreme v gorah in hribih") - nadmorska višina v vrsticah, čas (vsake
 * 3 ure, naslednja 2 dni) v vodoravno drsnih stolpcih. Izbranih je 6
 ...
 * koraka, le standardni nabor (975/950/925/900/850/800/700/600/500 ...).
 */
 ...
 * SkyTech zgodovini (glej pickThreeHourTicks zgoraj).
 */
...
/**
 * Barva glede na jakost vetra po višini - ločena lestvica od
 * rateWindClient (tista je umerjena za prizemni jadralni polet 8-30 km/h,
 * tu pa gre lahko za jetstream čez 100 km/h) - groba orientacija:
 * moder=mirno, rumen=zmerno, oranžen=močno, rdeč=zelo močno.
 */
